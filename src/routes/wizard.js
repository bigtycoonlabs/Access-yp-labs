...
router.post('/submit', [
  ...
  try {
    ...
    // 1. Upsert user account (wizard creates a pending account)
    let userResult = await client.query(
      'SELECT id FROM users WHERE email = $1', [contact.email]
    );
    let userId;
    if (!userResult.rows.length) {
      const newUser = await client.query(
        `INSERT INTO users (email, name, phone, business_name, role, status, created_at)
         VALUES ($1, $2, $3, $4, 'client', 'pending', NOW()) RETURNING id`,
    ...
    // 3. Save wizard submission
    const subResult = await client.query(
      `INSERT INTO wizard_submissions
         (user_id, track, housing_type, channel, mission, brand_color, payment_plan, subtotal, created_at)
    ...
    const projResult = await client.query(
      `INSERT INTO projects
         (user_id, wizard_submission_id, track, status, payment_plan, subtotal,
          financed_total, installment_amount, hosting_monthly, created_at)
       VALUES ($1,$2,$3,'awaiting_specialist_contact',$4,$5,$6,$7,$8,NOW()) RETURNING id`,
    ...
    for (const item of lineItems) {
      const deadline = item.sla_hours
        ? new Date(Date.now() + item.sla_hours * 3600 * 1000)
        : null;
      await client.query(
        `INSERT INTO order_line_items (project_id, service_type, label, amount, sla_hours, deadline, status, created_at)
         VALUES ($1,$2,$3,$4,$5,$6,'queued',NOW())`,
        [projectId, item.type, item.label, item.amount, item.sla_hours, deadline]
      );
    }

    // 6. Create pending payment plan record
    await client.query(
      `INSERT INTO payment_plans (project_id, plan_type, subtotal, financed_total, installment_amount,
         installments_total, installments_paid, status, created_at)
       VALUES ($1,$2,$3,$4,$5,$6,0,'pending_confirmation',NOW())`,
      [projectId, payment_plan, subtotal, financedTotal, installmentAmount, payment_plan === 'financed' ? installmentMonths : 1]
    );

    await client.query('COMMIT');

    // TODO: trigger Slack/email notification to specialist team

    res.status(201).json({
      message: 'Submission received. A YP Labs specialist will contact you within one business day.',
