...
router.post('/quote', [
  body('track').isIn(['A', 'B']),
  body('payment_plan').optional().isIn(['one_time', 'financed']),
], (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) return res.status(400).json({ errors: errors.array() });
  try {
    res.json(calculateQuote({ track: req.body.track, cart: req.body.cart || [], payment_plan: req.body.payment_plan || 'financed' }));
  } catch (error) {
...
router.post('/submit', [
  ...
  try {
    ...
    if (!userResult.rows.length) {
      const newUser = await client.query(`INSERT INTO users (email, name, phone, business_name, role, status, created_at) VALUES ($1, $2, $3, $4, 'client', 'pending', NOW()) RETURNING id`, [contact.email, contact.name, contact.phone || null, contact.business_name || null]);
    ...
    const subResult = await client.query(`INSERT INTO wizard_submissions (user_id, track, housing_type, channel, mission, brand_color, payment_plan, subtotal, created_at) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,NOW()) RETURNING id`, [userId, track, housing_type || null, channel || null, mission || null, brand_color || null, payment_plan, subtotal]);
    const submissionId = subResult.rows[0].id;
    const projResult = await client.query(`INSERT INTO projects (user_id, wizard_submission_id, track, status, payment_plan, subtotal, financed_total, installment_amount, hosting_monthly, created_at) VALUES ($1,$2,$3,'awaiting_specialist_contact',$4,$5,$6,$7,$8,NOW()) RETURNING id`, [userId, submissionId, track, payment_plan, subtotal, financedTotal, installmentAmount, hostingMonthly]);
    ...
    res.status(201).json({ message: 'Submission received. Create a client login to save and manage this build request.', project_id: projectId, submission_id: submissionId, user_id: userId, quote });
  } catch (err) {
