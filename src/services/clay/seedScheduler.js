// Clay's auto-seed scheduler. When enabled by staff, it tops up the Exchange review queue on a
// gentle cadence — at most two concepts a WEEK, spaced out. Every seed still lands in 'in_review';
...
//     week's budget). seed.runSeed() then enforces its own hard WEEKLY_CAP and minority guard.
...
async function tick() {
      ...
      UPDATE seed_schedule
       ...
       WHERE id = TRUE
         ...
         AND (SELECT COUNT(*) FROM concepts
                -- A ROLLING SEVEN DAYS, matching seed.js. A calendar boundary would let the week's
                -- allowance land on Sunday night and the next week's on Monday morning, which is
                -- four seeds in twelve hours and the exact behaviour the cap exists to prevent.
                WHERE origin = 'clay_seed' AND created_at >= now() - interval '7 days') < daily_target
       RETURNING id`);
  ...
  try {
    ...
    if (!out || out.ok === false) {
      ...
      const DECLINED = {
        weekly_cap: 'Clay has already seeded his two for this week. This is the guardrail working, not a fault.',
...
async function status() {
  const r = await query(`
    SELECT s.enabled, s.daily_target, s.min_gap_minutes, s.last_seed_at,
      (SELECT COUNT(*) FROM concepts WHERE origin='clay_seed' AND created_at >= now() - interval '7 days')::int AS seeded_week,
  ...
  if (!row.enabled) {
  ...
  } else if (row.seeded_week >= row.daily_target) {
    row.will_seed = false;
    row.why = `Clay has seeded ${row.seeded_week} in the last seven days, which is his limit. He will start again once one of those is a week old.`;
    ...
    row.will_seed = true;
    row.why = `Clay will seed. ${row.seeded_week} of ${row.daily_target} done in the last seven days; `
      + `${human} live listing(s) belong to real creators.`;
  }
  return row;
}

// Staff update. Clamped to safe ranges; the target can never exceed the hard WEEKLY_CAP.
// The column is still called daily_target for now — renaming it is a migration, and a migration to
// rename a column while the meaning is already correct is churn without a reader benefit.
async function configure({ enabled, dailyTarget, minGapMinutes } = {}) {
  ...
  if (Number.isInteger(dailyTarget)) { sets.push('daily_target=$' + (i++)); vals.push(Math.max(1, Math.min(seed.WEEKLY_CAP, dailyTarget))); }
