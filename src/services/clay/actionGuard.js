...
function claimedCompletedActions(text) {
  ...
  // LISTED on the marketplace / Dreamhold
  const listed =
    /\b(i'?ve|i have|i just|i'?ve gone ahead and)\b[^.!?]{0,45}\b(listed|posted|published|put it up|put your (concept|idea) up)\b[^.!?]{0,30}\b(marketplace|for sale|dreamhold|listing)\b/.test(t) ||
    /\byour (concept|idea|listing)\b[^.!?]{0,20}\b(is|are)\b[^.!?]{0,8}\b(now )?(live|listed|posted|published|up)\b[^.!?]{0,18}\b(marketplace|for sale|dreamhold)\b/.test(t) ||
    /\bit'?s (now )?(live|listed|up|published) (on the )?(marketplace|for sale|dreamhold)\b/.test(t);
