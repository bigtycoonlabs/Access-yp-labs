...
test('the magazine prints display names, never real names', () => {
...
test('the top builder is measured on turning up, not on achieving', () => {
  ...
  assert.ok(!/c\.title/.test(src.split('async function topDreamer')[1].split('}')[0] || ''),
    'the builder section never names a project');
});
