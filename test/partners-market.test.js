...
test('offering to help requires a display name first', () => {
  assert.match(src, /need_builder_tag/, 'the client is told exactly what is missing');
  assert.match(src, /display name/i);
  // The gate must sit BEFORE the interest row is written, or someone lands on the board nameless.
  const gateAt = src.indexOf('need_builder_tag');
...
test('Clay only suggests opportunities to people who opted in', () => {
  ...
  assert.match(onBlock, /need_builder_tag/);
});
