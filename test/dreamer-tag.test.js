...
test('changing a display name asks first, because it changes how people recognise you everywhere', () => {
  assert.strictEqual(spine.requiresConfirmation('set_builder_tag'), true);
  // Reading it never needs a confirmation — that would make Clay annoying for no benefit.
  assert.strictEqual(spine.requiresConfirmation('get_builder_tag'), false);
});
...
test('Clay is told to check before asking, and to ask only once', () => {
  ...
  assert.match(agent, /get_builder_tag first/i, 'he must not nag someone who already has one');
